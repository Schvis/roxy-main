...
async function main(): Promise<void> {
  ...
  check('tts disabled by default', repo.getSettings().ttsEnabled === false)
  repo.setTtsEnabled(true)
  check('setTtsEnabled persists', repo.getSettings().ttsEnabled === true)
  repo.setIdeMode(true)
  check('tts disabled by default when on IDE mode', repo.getSettings().ttsEnabled === false)
  repo.setTtsEnabled(true)
  check('tts can be explicitly enabled in IDE mode', repo.getSettings().ttsEnabled === true)
  repo.setIdeMode(false)
  check('tts enabled state restores when leaving IDE mode', repo.getSettings().ttsEnabled === true)
  check('tts auto start defaults to false', repo.getSettings().ttsAutoStart === false)
  repo.setTtsAutoStart(true)
  check('setTtsAutoStart persists', repo.getSettings().ttsAutoStart === true)
  check('tts mode defaults to all', repo.getSettings().ttsMode === 'all')
  repo.setTtsMode('sentence')
  check('setTtsMode persists', repo.getSettings().ttsMode === 'sentence')
  check('tts translate defaults to true', repo.getSettings().ttsTranslate === true)
  repo.setTtsTranslate(false)
  check('setTtsTranslate persists', repo.getSettings().ttsTranslate === false)
  check('tts lang defaults to ja', repo.getSettings().ttsLang === 'ja')
  repo.setTtsLang('en')
  check('setTtsLang persists', repo.getSettings().ttsLang === 'en')
  check('tts speed defaults to 15', repo.getSettings().ttsSpeed === 15)
  repo.setTtsSpeed(25)
  check('setTtsSpeed persists', repo.getSettings().ttsSpeed === 25)
  ...
  check('vtuber show chat bubble default true', repo.getSettings().vtuberShowChatBubble === true)
  repo.setVtuberShowChatBubble(false)
  check('setVtuberShowChatBubble persists', repo.getSettings().vtuberShowChatBubble === false)
  check('vtuber show status default true', repo.getSettings().vtuberShowStatus === true)
  repo.setVtuberShowStatus(false)
  check('setVtuberShowStatus persists', repo.getSettings().vtuberShowStatus === false)
  check('vtuber follow cursor default true', repo.getSettings().vtuberFollowCursor === true)
  repo.setVtuberFollowCursor(false)
  check('setVtuberFollowCursor persists', repo.getSettings().vtuberFollowCursor === false)
