...
export interface ChestAudioResult {
  audioBase64: string
  format: string
  name: string
  text?: string
...
export function getChestAudioDir(): string | null {
  const candidates = [
    path.join(process.cwd(), 'resources', 'voicelines', 'chest'),
    path.join(app.getAppPath(), 'resources', 'voicelines', 'chest')
  ]

  if (app.isPackaged) {
    candidates.unshift(
      path.join(process.resourcesPath, 'voicelines', 'chest'),
      path.join(process.resourcesPath, 'resources', 'voicelines', 'chest')
    )
  }
...
export async function getRandomChestAudio(): Promise<ChestAudioResult | null> {
  const dir = getChestAudioDir()
  if (!dir) return null
  ...
  try {
    ...
    console.warn('[Chest] Failed to read chest audio:', err)
    return null
  }
}
