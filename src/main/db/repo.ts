...
function saveProviderConnection(input: ConnectProviderInput): string {
  ...
  if (existing) {
    ...
    db.prepare(
      'UPDATE providers SET name = ?, base_url = ?, default_model = ?, discover_image_models = ?, enabled = 1 WHERE id = ?'
    ).run(name, baseURL, defaultModel, discoverImageModels, existing.id)
    return existing.id
  }
  const counter = db
    .prepare(
  ...
  db.prepare(
    ...
    INSERT INTO providers(id, seed_id, account_number, name, wire, auth, base_url,
      default_model, discover_image_models, enabled, sort_order, created_at)
    VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)
  `
  ).run(
    id,
