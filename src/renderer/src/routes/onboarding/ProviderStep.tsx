        ...
        <ProviderSetup
          ...
          connectionId={
            setupId === 'openai-compatible'
              ? undefined
              : providers.find((provider) => provider.seedId === setupId)?.id
          }
...
export function ProviderSetup({
  ...
  const [discoverImageModels, setDiscoverImageModels] = useState(
    existing?.discoverImageModels ?? false
  )
  ...
  const connect = async (allowUnverified = false): Promise<void> => {
    ...
    try {
      if (connectionId) clearModelCache(connectionId)
      const result = await api.providers.connect({
        ...
        defaultModel: isCustomCompatible ? defaultModel.trim() || undefined : undefined,
        discoverImageModels: isCustomCompatible ? discoverImageModels : undefined,
        allowUnverified
      })
      if (!result.ok) {
